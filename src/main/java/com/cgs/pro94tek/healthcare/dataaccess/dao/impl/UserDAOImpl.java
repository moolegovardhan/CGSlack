...
public class UserDAOImpl extends CustomHibernateDaoSupport  implements IUserDAO {
	...
	public User createOrUpdateUser(User userEntity) throws DataAccessException {
	 ...
	 public int checkUserId(String customerId) throws DataAccessException {
			...
			String queryUserByUserNameandPassword= "select count(user.id) from  User user where user.userName = :customerId";
			System.out.println("dao count.......:"+queryUserByUserNameandPassword);
		    Query query = getSession().createQuery(queryUserByUserNameandPassword);
